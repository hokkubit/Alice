...
def main(stdscr):
    ...
    def display_rows(menu, row_id: int, page_counter: int, menu_mode: str, height, width):
        ...
        if menu_mode == 'main':
            while True:
                ...
                if key == curses.KEY_UP and current_row_id > 0:
                    ...
                    current_row_id = 2
                    stdscr.refresh()
                elif key == curses.KEY_DOWN and current_row_id < len(display_menu.menu) - 1:
                    current_row_id += 1
                    stdscr.refresh()
                    ...
                    stdscr.refresh()
                # editor mode
                elif key == curses.KEY_ENTER or key in [10, 13] and current_row_id == len(display_menu.menu) - 3:
                    alice.edit_aleases(EDITOR)
                    stdscr.refresh()
                    display_rows(MAIN_MENU, 0, 1, 'main', height, width)

                # load first page of aliases
                elif key == curses.KEY_ENTER or key in [10, 13] and current_row_id == len(display_menu.menu) - 2:
                    if ALIASES:
                        stdscr.refresh()
                        chunk = alice.alias_paginate(ALIASES, current_page)
                        if chunk:
                            display_rows(chunk, 0, current_page, 'aliases', height, width)
                        else:
                            break
                    else:
                        stdscr.addstr(1, 0, "Sory, there are no aleases")
                        stdscr.getch()
                elif key == curses.KEY_ENTER or key in [10, 13] and current_row_id == len(display_menu.menu) - 1:
                    stdscr.clear()
                    break
                # Some adaptive, if we want to resize window update the func with the new h, w
                    ...
                    display_rows(menu, current_row_id, current_page, menu_mode, height, width)
                    break
        ...
        # Aliases menu navmode
            ...
            while True:
                ...
                # paginate to the previuos page of aleases
                    ...
                    if prev_chunk:
                        display_rows(prev_chunk, 0, current_page, 'aliases', height, width)
                    else:
                        break
                # DOWN condition
                elif key == curses.KEY_DOWN and current_row_id < len(display_menu.menu) - 1:
                ...
                # paginate to the next page of aleases
                    ...
                    if next_chunk:
                        display_rows(next_chunk, 0, current_page, 'aliases', height, width)
                    else:
                        break
                    ...
                    for id, row in enumerate(menu):
                        if id == current_row_id:
                            # alias_cmd = f'{str(row)} : {str(menu[row])}'
                            # stdscr.addstr(0, 0, alias_cmd)
                            cmdr = str(re.sub(r'[^\w\s]+|[\d]+', r'', f'{str(row)}').strip())
                            stdscr.addstr(0, 0, cmdr)
                            stdscr.addstr(3, 0, "Press Enter to exec")
                            stdscr.getch()
                            curses.endwin()
                            # getting os enviroment for source aliases
                            #TODO: check it in start script, for next usage
